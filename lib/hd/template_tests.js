...
System.init(requirements, function () {
    ...
    QUnit.test('Templating :: When Compiling Template Documents', function (assert) {
        var cdoc = System.Templating.parse('$if (true)$if (true)${content}$endif$endif');

        System.Templating.compile(cdoc);

        // First conditional
        assert.ok(is_set(cdoc.children[0].compiled), 'Should compile conditional statements.');

        // Second conditional
        assert.ok(is_set(cdoc.children[0].children[0].compiled), 'Should compile conditional statements.');

        // Var reference
        assert.ok(is_set(cdoc.children[0].children[0].children[0].compiled), 'Should compile var statements.');
    });
