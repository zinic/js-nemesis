...
System.init(requirements, function () {
    ...
    QUnit.test('Templating :: When Using No Features', function (assert) {
        var cdoc = System.Templating.parse('template text');

        assert.equal(cdoc.children.length, 1, 'Should produce one root child.');

        var child = cdoc.children[0];
        assert.equal(child.type, 'string', 'Should produce one child of type string.');
        assert.equal(child.payload, 'template text', 'Should have expected string contents.');
    });
    ...
    QUnit.test('Templating :: When Executing Template Documents', function (assert) {
        var cdoc = System.Templating.parse('$if (true)$if (true)${content}$endif$endif');
        var result = System.Templating.execute(cdoc, {'content': 'testing'});
