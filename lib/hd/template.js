(function () {
	...
	function TemplateManager() {
			...
			if (loaded_templates[name] == undefined) {
				...
				$.ajax({
					...
					success: function (data) {
						loaded_templates[name] = self.parse(data);
						on_completion();
					},
		...
		this.walk = function (tdoc, visitor) {
			...
			if (visitor(tdoc)) {
				while (depth_stack.length > 0) {
					...
					if (is_set(next.node.children) && next.index < next.node.children.length) {
		...
		this.render = function (name, scope) {
			return this.execute(this.get(name), scope);
		};

		this.execute = function (tdoc, scope) {
			var output = '';
			...
			// Set some common scope items
			scope['is_set'] = is_set;
			scope['is_not_set'] = is_not_set;
			...
			this.walk(tdoc, function (node) {
				...
				switch (node.type) {
					case 'condition':
						...
						descend = System.isolate(node.payload, scope);
						break;

					case 'var':
						output += System.isolate(node.payload, scope);
						break;
			...
			for (var i in template) {
				...
				switch (manager.state) {
					case S_START:
						...
						// Since we just started, fallthrough and treat this
						// character as string data.
					...
					case S_TOKEN_START:
						...
						// Fallthrough to the next state since we don't
						// rerun this switch.
